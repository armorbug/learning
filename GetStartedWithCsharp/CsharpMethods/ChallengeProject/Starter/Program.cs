// challenge project 4 - create a mini-game
...
// - The game is playable. The following features have been added:
...
// - Fixed an issue in the exercise where food will not spawn
//   if the player is (^_^) and food was consumed  
...
while (!shouldExit)
{
    ...
    //Move(nondirectionalDetect: true);
    if (HappyCheck())
    {
        Move();
        // speed up movement only if left or right 
        if (lastKey == ConsoleKey.LeftArrow || lastKey == ConsoleKey.RightArrow)
        {
            for (int i = 0; i < 3; i++)
            {
                if (PlayerAteFood(playerX, playerY, foodX, foodY)) break;
                Move(lastKey);
            }
        }
    }
...
//Returns true if the player ate the food
...
//Returns true if the player matches the (X_X) appearance 
bool FreezeCheck()
{
    return (player == "(X_X)") ? true : false;
}

//Returns true if the player matches the (^-^) appearance 
bool HappyCheck()
{
...
// If keyPress is not defined, run with current key press
// If keyPress is defined, run with last key press
void Move(ConsoleKey keyPress = ConsoleKey.NoName, bool nondirectionalDetect = false)
{
    int lastX = playerX;
    int lastY = playerY;

    if (keyPress == ConsoleKey.NoName)
    ...
    switch (keyPress)
    {
        ...
        case ConsoleKey.Escape:
            ...
            break;
        default:
            if (nondirectionalDetect) shouldExit = true;
            break;
    }
